...
- `src/core`: 안전시간·위험도·매칭·StatusCheck·순차 에스컬레이션·종료 결정. 누락·오류 안전시간 입력은 `UNKNOWN`으로 처리
...
Canonical DEMO 정책 식별자는 `00000000-0000-0000-0000-000000000001`, 이름은 `DEMO_ONLY_DEFAULT`, 버전은 `1`이다.
...
`DEMO_ONLY_RISK_POLICY`의 `0.5 / 0.2` 비율 기준과 응답 제한 10초는 제품·의료 담당자의 확정값이 아니다. 승인된 설정으로 교체하기 전에는 운영 판단에 사용하면 안 된다. 이 엔진은 등록된 전원 정보 기반 추정만 하며 의료 상태를 진단하지 않는다.
